<div>
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            ...
            <a href="{{ route('admin.scholarship-batches.create') }}" wire:navigate class="mt-4 sm:mt-0 px-6 py-3 bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700 transition duration-150 ease-in-out flex items-center">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900">{{ $statistics['total_batches'] ?? 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-green-600">{{ $statistics['active_batches'] ?? 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-yellow-600">{{ $statistics['upcoming_batches'] ?? 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-red-600">{{ $statistics['closed_batches'] ?? 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-purple-600">{{ $statistics['total_submissions'] ?? 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center mb-2 sm:mb-0">
                        <span class="text-sm text-gray-700 font-medium">{{ count($selectedBatches) }} batch(es) selected</span>
                    </div>
                    ...
                    @forelse ($batches as $batch)
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" wire:click="toggleBatchSelection({{ $batch->id }})"
                                       @if(in_array($batch->id, $selectedBatches)) checked @endif
                                       class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                            </td>

                            {{-- Batch Name with enhanced display --}}
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    <a href="{{ route('admin.scholarship-batches.submissions', $batch) }}" wire:navigate
                                       class="text-sm font-medium text-gray-900 hover:text-indigo-600">
                                        {{ $batch->name }}
                                    </a>
                                    @if($batch->description)
                                        <span class="text-xs text-gray-500 mt-1">{{ Str::limit($batch->description, 50) }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col space-y-1">
                                    ...
                                    @switch($batch->status)
                                        ...
                                        @case('upcoming') bg-yellow-100 text-yellow-800 @break
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-xs text-gray-500">
                                        ...
                                        @elseif($isUpcoming)
                                            <span class="text-yellow-600">● Starts in {{ $batch->start_date->diffForHumans() }}</span>
                                        @else
                                            <span class="text-red-600">● Ended {{ $batch->end_date->diffForHumans() }}</span>
                                        @endif
                                    </span>
                                </div>
                            </td>

                            {{-- Enhanced Submissions Count --}}
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex flex-col items-center">
                                    <span class="text-lg font-semibold text-gray-900">{{ $batch->submissions_count }}</span>
                                    <div class="flex space-x-2 text-xs">
                                        @if($batch->pending_submissions_count > 0)
                                            <span class="text-yellow-600">{{ $batch->pending_submissions_count }} pending</span>
                                        @endif
                                        @if($batch->approved_submissions_count > 0)
                                            <span class="text-green-600">{{ $batch->approved_submissions_count }} approved</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    ...
                                    <a href="{{ route('admin.scholarship-batches.results', $batch) }}" wire:navigate
                                    ...
                                    {{-- Edit Batch --}}
                                    <a href="{{ route('admin.scholarship-batches.edit', $batch) }}" wire:navigate
                                    ...
                                    {{-- Status Actions --}}
                                    @if($batch->status !== 'active' && !$isClosed)
                                        <button wire:click="confirmActivateBatch({{ $batch->id }})"
                                    ...
                                    @if($batch->status !== 'closed')
                                        <button wire:click="confirmCloseBatch({{ $batch->id }})"
                                    ...
                                    <button wire:click="deleteBatch({{ $batch->id }})"
                                            wire:confirm="Are you sure you want to delete '{{ $batch->name }}'? All related submissions will also be deleted."
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-8 whitespace-nowrap text-sm text-gray-500 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <a href="{{ route('admin.scholarship-batches.create') }}" wire:navigate class="mt-4 px-5 py-2.5 bg-indigo-500 text-white rounded-md hover:bg-indigo-600 transition duration-150 ease-in-out text-sm font-medium">
                                        Create First Batch
                                    </a>
                                </div>
                            </td>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-6 rounded-lg shadow-xl max-w-sm mx-auto">
                ...
                <p class="text-sm text-gray-600 mb-6">
                    ...
                    <strong>{{ $batches->find($batchIdToClose)?->name }}</strong>?
                    <br><br>
    ...
    {{-- Activate Batch Confirmation Modal --}}
    @if ($confirmingActivateBatch)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-6 rounded-lg shadow-xl max-w-sm mx-auto">
                <div class="flex items-center mb-4">
                    <div class="flex-shrink-0">
                ...
                <p class="text-sm text-gray-600 mb-6">
                    ...
                    <strong>{{ $batches->find($batchIdToActivate)?->name }}</strong>?
                    <br><br>
    ...
    @if ($confirmingBulkDelete)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-6 rounded-lg shadow-xl max-w-md mx-auto">
                ...
                <p class="text-sm text-gray-600 mb-6">
                    Are you sure you want to delete <strong>{{ count($batchesToDelete) }}</strong> selected scholarship batches?
                    <br><br>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button wire:click="bulkDelete" type="button"
                        ...
                        Delete {{ count($batchesToDelete) }} Batches
                    </button>
                </div>
            </div>
        </div>
